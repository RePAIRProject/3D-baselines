...
class GeometryPartDataset(Dataset):
    ...
    def _read_data(self, data_fn):
        print('\n multipart normalization + no pivot............')
    ...
    def _get_pcs(self, data_folder):
        ...
        data_folder_orig = str(data_folder)
        ...
        pcs_paths = [f'{data_folder_orig}/{mesh_fil}' for mesh_fil in mesh_files]
        pcs, min_values, max_values = self.normalize_point_cloud(pcs)   # by adeela
        return np.stack(pcs, axis=0), min_values, max_values, pcs_paths

    def __getitem__(self, index):
        pcs, min_values, max_values, pcs_paths = self._get_pcs(self.data_list[index])
