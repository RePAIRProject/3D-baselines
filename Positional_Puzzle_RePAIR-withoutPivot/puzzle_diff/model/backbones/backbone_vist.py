...
class Eff_GAT_Vist(nn.Module):
    ...
    def __init__(
        ...
        self.visual_backbone = timm.create_model(
            "efficientnet_b0", pretrained=pretrained_backbone, features_only=True
        )
        self.with_images = with_images
        self.with_text = with_text

        if with_images:
            visual_backbone_feat_dim = 2560
        else:
            visual_backbone_feat_dim = 0
        if with_text:
            text_feat_dim = {"facebook/bart-base": 768, "facebook/bart-large": 1024}[
                model
            ]
        else:
            text_feat_dim = 0
    ...
    def forward_with_feats(
        ...
        combined_feats = [pos_feats, time_feats]
        if self.with_text:
            combined_feats += [text_feats]
        if self.with_images:
            combined_feats += [frames_feats]

        combined_feats = torch.cat(combined_feats, -1)
        combined_feats = self.mlp(combined_feats)

        # GNN
        feats, _ = self.gnn_backbone(x=combined_feats, edge_index=edge_index)
