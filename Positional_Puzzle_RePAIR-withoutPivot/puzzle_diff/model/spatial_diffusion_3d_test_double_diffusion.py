...
class GNN_Diffusion(pl.LightningModule):
    ...
    def initialize_torchmetrics(self, categories):
        ...
        for i in categories:
            ...
            metrics[f"precision_{i}"] = torchmetrics.MeanMetric()
            metrics[f"recall_{i}"] = torchmetrics.MeanMetric()
            metrics[f"f1_{i}"] = torchmetrics.MeanMetric()
            metrics[f"Qpos_{i}"] = torchmetrics.MeanMetric()
        ...
        metrics_AVG[f"precision_AVG"] = torchmetrics.MeanMetric()
        metrics_AVG[f"recall_AVG"] = torchmetrics.MeanMetric()
        metrics_AVG[f"f1_AVG"] = torchmetrics.MeanMetric()
        metrics_AVG[f"Qpos_AVG"] = torchmetrics.MeanMetric()
    ...
    def validation_step(self, batch, batch_idx):
        with torch.no_grad():
            ...
            min_vals = batch.denorm_min_vals
            max_vals = batch.denorm_max_vals
            ...
            for i in range(batch.batch.max() + 1):
                ...
                part_acc, prec_calc, recall_calc, f1_calc, Qpos = calc_part_acc(pcds.cpu(), pred_t.cpu(), gt_t.cpu(), pred_r.cpu(), gt_r.cpu(), None, min_vals,  max_vals)
                ...
                self.metrics[f"precision_{cat}"].update(prec_calc)
                self.metrics[f"recall_{cat}"].update(recall_calc)
                self.metrics[f"f1_{cat}"].update(f1_calc)
                self.metrics[f"Qpos_{cat}"].update(Qpos)
        ...
        with torch.no_grad():
            ...
            for i in range(batch.batch.max() + 1):
                ...
                if self.save_eval_images:
                    ...
                    mesh_dir_path = (
                        Path("datasets")
                        / "RePAIR_assembled_objects_dataset"
                    ...
                    bb_path.append(f"{rot_metric.item()}rot-{t_metric.item()}tras-{part_acc.item()}pa-{bb_path[1]}")
